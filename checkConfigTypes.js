...
function checkDefined(layoutText) {
  ...
  if (!layoutText.contributorLinks) {
    throw TypeError(`${configFileString} has no "contributorLinks" property`);
  }
}
...
function checkPropertyTypes(layoutText) {
  ...
  if (!Array.isArray(layoutText.contributorLinks)) {
    throw TypeError(`${configFileString} "contributorLinks" property is not an array`);
  }
}
...
function main() {
  ...
  const layoutTextUnparsed = fs.readFileSync(filePath, { encoding: 'utf8' });
  ...
  checkArrayPropertyTypes(layoutText.contributorLinks, 'contributorLinks');
}
